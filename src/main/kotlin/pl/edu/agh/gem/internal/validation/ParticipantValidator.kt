...
class ParticipantValidator : BaseValidator<ExpenseDataWrapper>() {
    override val checks: List<Check<ExpenseDataWrapper>> = listOf(
        Check(USER_NOT_PARTICIPANT) { validateIfUserIsParticipant(it) },
        Check(DUPLICATED_PARTICIPANT) { validateIfParticipantsAreUnique(it) },
        Check(PARTICIPANT_MIN_SIZE) { validateParticipantsSize(it) },
        Check(PARTICIPANT_NOT_GROUP_MEMBER) { validateIfParticipantsAreGroupMembers(it) },
    )
    ...
    private fun validateIfParticipantsAreUnique(expenseDataWrapper: ExpenseDataWrapper): Boolean {
        return expenseDataWrapper.expense.expenseParticipants.let {
            it.distinctBy { participant -> participant.participantId }.size == it.size
        }
    }
    ...
    private fun validateIfParticipantsAreGroupMembers(expenseDataWrapper: ExpenseDataWrapper): Boolean {
        val membersIds = expenseDataWrapper.group.members.members.map { it.id }
        return expenseDataWrapper.expense.expenseParticipants
            .all { it.participantId in membersIds }
    }
}
